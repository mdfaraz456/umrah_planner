...
$userData = $userObj->getAllUser('Agent');
$db = new dbClass();
...
if (isset($_GET['status'])) {
  ...
  $sqlStatus = $db->execute("UPDATE users SET is_active = '$status' WHERE user_id  = '$id'");
...
if (isset($_REQUEST['delete']) && $_REQUEST['delete'] == 'y') {
  ...
  $filename = $userObj->getUserById($id);
 

  $sqlDelete = $db->execute("DELETE FROM users WHERE user_id  = '$id'");
  if ($sqlDelete == true):
      ...
      <div class="container-fluid">
        ...
        <div class="row page-titles">
          <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">All Vendor</h3>
            <ol class="breadcrumb">
          ...
          <a href="agent.php"> <button type="button" class="btn btn-info"> <i class="fa fa-plus-circle"></i>  Create New </button></a>
          </div>
          <div class="">
            <button class="
                      right-side-toggle
                  ...
                  <table id="example23" class="
                    ...
                    <tbody>
                      ...
                      foreach ($userData as $users): ?>
                        <tr>
                          <td><?php echo $i++; ?></td>
                          <td><a href="details.php?id=<?php echo base64_encode($users['user_id'] ?? '') ?>">
                          ...
                          <td class="icon-dlt-edit ">
                            <?php if ($users['is_active'] ?? '' == 1): ?>
                              <a href="?status=0&id=<?php echo $users['user_id'] ?? ''; ?>">
                                <i class="fa-solid fa-user" style="color: #e41111;"></i>
                              </a>
                            <?php else: ?>
                              <a href="?status=1&id=<?php echo $users['user_id'] ?? ''; ?>">
                                <i class="fa-solid fa-user-slash" style="color: #d21414;"></i>
                              </a>
                            <?php endif; ?>

                            <a href="agent.php?id=<?php echo base64_encode($users['user_id'] ?? '') ?>"><i class="fa-solid fa-pencil" style="color: #e81111;"></i></a>
                            <a href="?id=<?php echo $users['user_id']; ?>&delete=y" onClick="return confirm('Are you sure !! Record will be delete parmanently ..!!')"><i class="fa-solid fa-trash" style="color: #f50000;"></i> </a>
                          </td>
                        </tr>
                      <?php endforeach; ?>

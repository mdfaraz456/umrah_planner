...
$userData = $userObj->getAllCustomer();
$db = new dbClass();
...
if (isset($_GET['status'])) {
  ...
  $sqlStatus = $db->execute("UPDATE customers SET is_active = '$status' WHERE customer_id  = '$id'");
...
if (isset($_REQUEST['delete']) && $_REQUEST['delete'] == 'y') {
  ...
  $filename = $userObj->getCustomerById($id);
  ...
  foreach ($files as $file) {
    if (file_exists('../uploads/customer/' . $filename[$file])) {
      unlink('../uploads/customer/' . $filename[$file]);
    }
  }
                  ...
                  <table id="example23" class="
                    ...
                    <tbody>
                      ...
                      foreach ($userData as $users): ?>
                        <tr>
                          <td><?php echo $i++; ?></td>
                          <td><a href="customer-details.php?id=<?php echo base64_encode($users['customer_id'] ?? '') ?>">
                              <?php echo $users['customer_name'] ?? ''; ?></a>
                          </td>
                          <td><?php echo $users['customer_number'] ?? ''; ?></td>
                          <td><?php echo $users['customer_email'] ?? ''; ?></td>
                          <td><?php echo $users['customer_password'] ?? ''; ?></td>
                          <td><?php echo $users['customer_gender'] ?? ''; ?></td>
                          <td><?php echo $users['customer_nationality'] ?? ''; ?></td>
                          <td><?php echo $users['customer_state'] ?? ''; ?></td>

                          <td class="icon-dlt-edit ">
                            <?php if ($users['is_active'] ?? '' == 1): ?>
                              <a href="?status=0&id=<?php echo $users['customer_id'] ?? ''; ?>">
                                <i class="fa-solid fa-user" style="color: #e41111;"></i>
                              </a>
                            <?php else: ?>
                              <a href="?status=1&id=<?php echo $users['customer_id'] ?? ''; ?>">
                                <i class="fa-solid fa-user-slash" style="color: #d21414;"></i>
                              </a>
                            <?php endif; ?>

                            <a href="user.php?id=<?php echo base64_encode($users['customer_id'] ?? '') ?>"><i class="fa-solid fa-pencil" style="color: #e81111;"></i></a>
                            <a href="?id=<?php echo $users['customer_id']; ?>&delete=y" onClick="return confirm('Are you sure !! Record will be delete parmanently ..!!')"><i class="fa-solid fa-trash" style="color: #f50000;"></i> </a>
                          </td>
                        </tr>
                      <?php endforeach; ?>
